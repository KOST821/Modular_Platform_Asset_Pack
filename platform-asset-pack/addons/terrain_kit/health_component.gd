...
func _ready() -> void:
	...
	if user != null : return
	user = get_parent()
	printerr('You forgot to place the user to the health component (',self,'). Automaticlly added the parent (',user,') KOSSAINIS')
...
func death()->void:
	...
	if post_death_action == PostDeath.DestroyNode:
		# Defer the deletion of the single node
		user.call_deferred("queue_free")
		
	elif post_death_action == PostDeath.RestartScene:
		# Defer the deletion of the entire scene
